...
function getCompletePokemonData() {
  ...
  const registeredNamesSet = new Set(registeredNames);
  ...
  try {
    ...
    const filteredData = allPokemonData.filter((pokemon) =>
      registeredNamesSet.has(pokemon[2]) // O(1) lookup with Set
    );
...
// Optimized: No server-side image checking - return most likely URL and let client handle fallbacks
function getImageUrl(pokemonName, pokemonId) {
    ...
    const baseFileName = `${paddedId}-${sanitizedName}`;
    // Return most likely format (.png) - client will handle fallbacks
    return `${IMG_BASE_URL}${baseFileName}.png`;
}
...
function updateTrainerDataInSheet(newTrainerData) {
    ...
    const trainerName = newTrainerData[TRAINER_COLUMN_INDICES.name];

    for (let i = 1; i < data.length; i++) {
        if (data[i][TRAINER_COLUMN_INDICES.name].toLowerCase() === trainerName.toLowerCase()) {
            const row = i + 1; // Row number in Google Sheets (1-based index)
...
function doGet(e) {
    const template = HtmlService.createTemplateFromFile('master');
...
function getTrainerPokemon(trainerName) {
    ...
    const pokemonList = [];
    for (let i = 1; i < data.length; i++) {
        if (data[i][POKEMON_COLUMN_INDICES.trainername] === trainerName) {
            pokemonList.push({
                name: data[i][POKEMON_COLUMN_INDICES.name],
                level: data[i][POKEMON_COLUMN_INDICES.level],
                image: data[i][POKEMON_COLUMN_INDICES.image]
            });
        }
    }
    return pokemonList;
...
function getTrainers() {
    ...
    for (let i = 1; i < data.length; i++) {
        ...
        trainers.push({
            id: i,
            image: row[TRAINER_COLUMN_INDICES.image],
            name: row[TRAINER_COLUMN_INDICES.name],
            pinCode: row[TRAINER_COLUMN_INDICES.pinCode]
        });
    }
    return trainers;
}
...
function updateActivePartyStatus(trainerName, pokemonName, pokeslots, action) {
    ...
    for (let i = 1; i < data.length; i++) {
        if (data[i][POKEMON_COLUMN_INDICES.trainername] === trainerName &&
            data[i][POKEMON_COLUMN_INDICES.inactiveparty]) {
            activeParty.push(parseInt(data[i][POKEMON_COLUMN_INDICES.inactiveparty], 10));
        }
    }

    activeParty.sort((a, b) => a - b); // Sort by slot number
    // Optimized: Use Set for O(1) lookup
    const activePartySet = new Set(activeParty);
    ...
    if (action === 'add') {
        if (activeParty.length >= pokeslots) {
            ...
            for (let i = 1; i < data.length; i++) {
                if (data[i][POKEMON_COLUMN_INDICES.trainername] === trainerName &&
                    data[i][POKEMON_COLUMN_INDICES.name] === pokemonName) {
                    POKEMON_DATA_SHEET.getRange(i + 1, POKEMON_COLUMN_INDICES.inactiveparty + 1).setValue(nextSlot);
                    break;
                }
            }
        ...
        for (let i = 1; i < data.length; i++) {
            if (data[i][POKEMON_COLUMN_INDICES.trainername] === trainerName &&
                data[i][POKEMON_COLUMN_INDICES.name] === pokemonName) {
                POKEMON_DATA_SHEET.getRange(i + 1, POKEMON_COLUMN_INDICES.inactiveparty + 1).setValue('');
                break;
            }
        }
...
function checkUtilitySlotStatus(trainerName) {
    ...
    for (let i = 1; i < data.length; i++) {
        if (data[i][POKEMON_COLUMN_INDICES.trainername] === trainerName &&
            data[i][POKEMON_COLUMN_INDICES.utilityslot] === 1) {
            return {
                hasUtilityPokemon: true,
                utilityPokemonName: data[i][POKEMON_COLUMN_INDICES.name]
            };
        }
    }
        ...
        if (action === 'add') {
            ...
            for (let i = 1; i < data.length; i++) {
                if (data[i][POKEMON_COLUMN_INDICES.trainername] === trainerName &&
                    data[i][POKEMON_COLUMN_INDICES.utilityslot] === 1 &&
                    data[i][POKEMON_COLUMN_INDICES.name] !== pokemonName) {
                    POKEMON_DATA_SHEET.getRange(i + 1, POKEMON_COLUMN_INDICES.utilityslot + 1).setValue('');
                }
            }
            ...
            for (let i = 1; i < data.length; i++) {
                if (data[i][POKEMON_COLUMN_INDICES.trainername] === trainerName &&
                    ...
                    if (data[i][POKEMON_COLUMN_INDICES.inactiveparty]) {
                        return {
                            status: 'error',
                    ...
                    POKEMON_DATA_SHEET.getRange(i + 1, POKEMON_COLUMN_INDICES.utilityslot + 1).setValue(1);
                    return { status: 'success' };
                }
            }
        } else if (action === 'remove') {
...
function calculateModifiers(newPokemonData, statIncreases, newFeats) {
    try {
        ...
        // Optimized: Use Sets for O(1) lookups instead of O(n) array.includes()
        const featsSet = new Set(feats);
        ...
        // Apply feats - Optimized with Set lookups
        if(newSkillsSet.has("Acrobat")){
          if (featsSet.has("Acrobat") && dexterity < 20) {
              dexterity += 1;
          }
          skills.push("Acrobatics");
        }

        if(newSkillsSet.has("Actor")){
          if (featsSet.has("Actor") && charisma < 20) {
              charisma += 1;
          }
        }

        if(newSkillsSet.has("Athlete")){
          if (featsSet.has("Athlete")) {
        ...
        if(newSkillsSet.has("Brawny")){
          if (featsSet.has("Brawny") && strength < 20) {
              strength += 1;
          }
          skills.push("Athletics");
        }

        if(newSkillsSet.has("Durable")){
          if (featsSet.has("Durable") && constitution < 20) {
              constitution += 1;
          }
        }

        if(newSkillsSet.has("Observant")){
          if (featsSet.has("Observant")) {
        ...
        if(newSkillsSet.has("Perceptive")){
          if (featsSet.has("Perceptive") && wisdom < 20) {
              wisdom += 1;
          }
          skills.push("Perception");
        }

        if(newSkillsSet.has("Quick-Fingered")){
          if (featsSet.has("Quick-Fingered") && dexterity < 20) {
              dexterity += 1;
          }
        }

        if(newSkillsSet.has("Resilient")){
          if (featsSet.has("Resilient")) {
        ...
        if(newSkillsSet.has("Stealthy")){
          if (featsSet.has("Stealthy") && dexterity < 20) {
              dexterity += 1;
          }
          skills.push("Stealth");
        }

        if(newSkillsSet.has("AC Up")){
          if (featsSet.has("AC Up")) {
        ...
        if(newSkillsSet.has("Tough")){
          if (featsSet.has("Tough")) {
              hp += level * 2;
          }
        }

        if(newSkillsSet.has("Tireless")){
          if (featsSet.has("Tireless")) {
        ...
        newPokemonData[30] = featsSet.has("Alert") ? dexmodifier + 5 : dexmodifier;
...
function loadAllGameData() {
    const startTime = Date.now();
    try {
        // Removed async/await - not beneficial in Google Apps Script
...
function writeItems(trainerName, newInventory) {
    try {
        const data = TRAINER_DATA_SHEET.getDataRange().getValues();
        const trainerRow = data.findIndex(row => row[TRAINER_COLUMN_INDICES.name] === trainerName);

        if (trainerRow === -1) {
            return { status: 'error', message: 'Trainer not found' };
        }

        TRAINER_DATA_SHEET.getRange(trainerRow + 1, TRAINER_COLUMN_INDICES.inventory + 1).setValue(newInventory);
        return { status: 'success' };
    } catch (error) {
...
function writeTrainerGear(trainerName, newGear) {
    try {
        ...
        TRAINER_DATA_SHEET.getRange(trainerRow + 1, TRAINER_COLUMN_INDICES.gear + 1).setValue(newGear);
        return { status: 'success' };
    } catch (error) {
...
function writePokemonGear(trainerName, pokemonName, newGear) {
    try {
        const data = POKEMON_DATA_SHEET.getDataRange().getValues();
        const pokemonRow = data.findIndex(row =>
            row[REGISTERED_POKEMON_COLUMN_INDICES.trainername] === trainerName &&
            row[REGISTERED_POKEMON_COLUMN_INDICES.name] === pokemonName
        );
        ...
        POKEMON_DATA_SHEET.getRange(pokemonRow + 1, REGISTERED_POKEMON_COLUMN_INDICES.gear + 1).setValue(newGear);
        return { status: 'success' };
    } catch (error) {
        Logger.log('Error writing gear: ' + error);
        return { status: 'error', message: 'Failed to write gear' };
...
function writeTrainerLiveStats(trainerName, stat, newValue) {
    try {
        ...
        if(stat === 'HP') {
          TRAINER_DATA_SHEET.getRange(trainerRow + 1, TRAINER_COLUMN_INDICES.currentHP + 1).setValue(newValue);
          return { status: 'success' };

        } else if(stat === 'VP'){
          TRAINER_DATA_SHEET.getRange(trainerRow + 1, TRAINER_COLUMN_INDICES.currentVP + 1).setValue(newValue);
          return { status: 'success' };

        } else { // AC
          TRAINER_DATA_SHEET.getRange(trainerRow + 1, TRAINER_COLUMN_INDICES.currentAC + 1).setValue(newValue);
          return { status: 'success' };
        }
    } catch (error) {
...
function writeMoney(trainerName, moneyAmount) {
    ...
    if (row) {
        try {
            sheet.getRange(row, TRAINER_COLUMN_INDICES.money + 1).setValue(JSON.stringify(moneyAmount));
        } catch (error) {
...
function writeComment(trainerName, pokemonName, comment) {
    try {
        ...
        POKEMON_DATA_SHEET.getRange(pokemonRow + 1, REGISTERED_POKEMON_COLUMN_INDICES.comment + 1).setValue(comment);
        return { status: 'success' };
    } catch (error) {
...
function writeAffinity(trainerName, affinity) {
    ...
    if (row) {
        sheet.getRange(row, TRAINER_COLUMN_INDICES.affinity + 1).setValue(affinity);
    }
}
...
function writeSpecialization(trainerName, specialization) {
    ...
    if (row) {
        sheet.getRange(row, TRAINER_COLUMN_INDICES.specialization + 1).setValue(specialization);
    }
}
...
function writeTrainerPath(trainerName, trainerPath) {
    ...
    if (row) {
        sheet.getRange(row, TRAINER_COLUMN_INDICES.trainerpath + 1).setValue(trainerPath);
    }
}
...
function writeTypeAwakening(trainerName, typeAwakeningData) {
    ...
    if (row) {
        sheet.getRange(row, TRAINER_COLUMN_INDICES.trainerpath + 1).setValue(typeAwakeningData);
    }
}
...
function getRegisteredPokemonList() {
  ...
  try {
    ...
    const lightweightData = allPokemonData
      .filter(pokemon => registeredNamesSet.has(pokemon[2]))
      .map(row => {
