  ...
  return new Promise((resolve) => {
    ...
    const sectionHtml = sections.map((sec, si) => {
      const rows = groupEffects(sec.entries).map((item, ii) => {
        ...
        if (item.type === 'row') {
          ...
          return `<div class="effects-popup-row">${_optionHtml(item.entry, id, 'checkbox', id, item.entry.verdict === 'yes')}${_diceInput(item.entry.effect, id)}</div>`;
        }
        ...
        const opts = item.entries.map(en => {
          const id = idOf(en);
          return _optionHtml(en, id, 'radio', gname, false) + _diceInput(en.effect, id);
        }).join('');
    ...
    overlay.addEventListener('click', (e) => {
      ...
      overlay.querySelectorAll('input[data-eid]').forEach(input => {
        ...
        const { targetId, effect } = byId.get(input.dataset.eid);
